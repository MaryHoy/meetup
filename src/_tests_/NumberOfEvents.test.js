...
describe('<NumberOfEvents /> component', () => {
  ...
  test('render text input correctly', () => {
    ...
    expect(NumberOfEventsWrapper.find('#numberOfEvents_input').prop('value')).toBe(numberOfEvents);
  });

  test('change state when input changes', () => {
    const eventObject = { target: { value: 32 }};
    NumberOfEventsWrapper.find('#numberOfEvents_input').simulate('change', eventObject);
